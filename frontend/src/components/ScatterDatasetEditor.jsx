...
export default function ScatterDatasetEditor({ datasets, onDatasetsChange }) {
  ...
  const addScatterPoint = (datasetIndex) => {
    const dataset = datasets[datasetIndex]
    const updated = datasets.map((ds, i) => {
      if (i === datasetIndex) {
        return {
          ...ds,
          data: [...ds.data, { x: 0, y: 0, label: `Punkt ${ds.data.length + 1}` }]
        }
      }
      return ds
    })
  ...
  const removeScatterPoint = (datasetIndex, pointIndex) => {
    const updated = datasets.map((ds, i) => {
      if (i === datasetIndex) {
        return {
          ...ds,
  ...
  const updateScatterPoint = (datasetIndex, pointIndex, field, value) => {
    const updated = datasets.map((ds, i) => {
      if (i === datasetIndex) {
  ...
  return (
    <div className="space-y-4">
                      ...
                      {dataset.data.map((point, pointIdx) => (
                        <div key={pointIdx} className="bg-dark-secondary rounded p-3 border border-gray-700/50">
                          <div className="flex items-start space-x-2">
                            <div className="flex-1 space-y-2">
                              <div>
                                ...
                                <input
                                  ...
                                  onChange={(e) => updateScatterPoint(dsIdx, pointIdx, 'label', e.target.value)}
                              ...
                              <div className="grid grid-cols-2 gap-2">
                                <div>
                                  ...
                                  <input
                                    type="number"
                                    value={point.x || 0}
                                    onChange={(e) => updateScatterPoint(dsIdx, pointIdx, 'x', e.target.value)}
                                ...
                                <div>
                                  ...
                                  <input
                                    type="number"
                                    value={point.y || 0}
                                    onChange={(e) => updateScatterPoint(dsIdx, pointIdx, 'y', e.target.value)}
                                    className="w-full px-2 py-1 bg-dark-secondary text-dark-textLight rounded border border-gray-700 focus:border-blue-500 focus:outline-none text-sm"
                                  />
                                </div>
                              </div>
                            ...
                            <button
                              onClick={() => removeScatterPoint(dsIdx, pointIdx)}
      ...
      <div className="text-xs text-dark-textGray bg-dark-bg rounded p-3">
        💡 <strong>Streudiagramm:</strong> X und Y bestimmen die Position des Punktes im Koordinatensystem. 
        Jeder Datensatz kann unterschiedlich viele Punkte haben. Klicke auf einen Datensatz und füge direkt dort neue Punkte hinzu.
      </div>
    </div>
  )
}
